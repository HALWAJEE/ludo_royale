...
function publicPlayer(p) {
  return { id: p.id, name: p.name, color: p.color, connected: !!p.connected };
}
function detachPlayer(room, player, permanent = false) {
  ...
  player.disconnectTimer = setTimeout(() => {
    ...
    if (!r || player.connected) return;
    r.players = r.players.filter(p => p !== player);
    if (!r.players.length) rooms.delete(r.id);
    else {
      if (r.hostId === player.id) r.hostId = r.players[0].id;
      // A long-disconnected player should not freeze the whole match.
      if (r.game?.started) {
        const gone = playerIndex(r, player.id);
        if (gone === r.game.turn) nextTurn(r);
        r.game.pawns = r.game.pawns.slice(0, r.players.length);
        r.game.turn = Math.min(r.game.turn, Math.max(0, r.players.length - 1));
      }
      broadcastState(r);
    }
  }, 60000);
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (m.type === 'create') {
      ...
      rooms.set(id, room);
      room.players.push({ ws, id: ws.id, token: crypto.randomUUID(), connected: true, disconnectTimer: null, name: String(m.name || 'Player').trim().slice(0, 16) || 'Player', color: COLORS[0] });
      ws.room = id;
      send(ws, { type: 'joined', selfId: ws.id, resumeToken: room.players.find(p => p.id === ws.id).token, state: state(room) });
      return;
    }
    ...
    if (m.type === 'join') {
      ...
      room.players.push({ ws, id: ws.id, token: crypto.randomUUID(), connected: true, disconnectTimer: null, name: String(m.name || 'Player').trim().slice(0, 16) || 'Player', color: COLORS[room.players.length] });
      ws.room = id;
      send(ws, { type: 'joined', selfId: ws.id, resumeToken: room.players.find(p => p.id === ws.id).token, state: state(room) });
      broadcastState(room);
      return;
    ...
    if (m.type === 'chat') {
      ...
      if (!text) return;
      const now = Date.now();
      if (player.lastChatAt && now - player.lastChatAt < 500) return;
      player.lastChatAt = now;
      broadcast(room, { type: 'chat', message: { id: crypto.randomUUID(), playerId: player.id, name: player.name, color: player.color, text, at: now } });
      return;
    }
    ...
    if (m.type === 'resume') {
      ...
      ws.room = room.id;
      ws.id = player.id;
      send(ws, { type: 'resumed', selfId: player.id, resumeToken: player.token, state: state(room) });
      broadcastState(room);
      return;
    }
