...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ roomCode, playerName }) => {
    ...
    if (room.gameStarted) return socket.emit('errorMsg', 'This game has already started.');
    if (room.players.length >= 4) return socket.emit('errorMsg', 'Room is already full (max 4 players).');
  ...
  socket.on('rollDice', ({ roomCode }) => {
    ...
    if (validMoves.length === 0) {
      room.statusMessage = `${currentActivePlayer.name} rolled ${roll}. No valid moves!`;
      setTimeout(() => advanceTurn(room), 1300);
    } else {
      room.statusMessage = `${currentActivePlayer.name} rolled ${roll}. Tap a pawn to move.`;
      setTimeout(() => {
        io.to(roomCode).emit('gameStateUpdate', room);
      }, 600);
    }
  ...
  socket.on('moveToken', ({ roomCode, tokenId }) => {
        ...
        room.tokens.forEach((other) => {
          if (other.playerId !== token.playerId && other.step >= 0 && other.step <= 50) {
            ...
            if (targetGlobal === otherGlobal) {
              ...
              room.statusMessage = `${currentActivePlayer.name} captured an opponent pawn!`;
            }
          }
        });
      }
    ...
    if (finished === 4) {
      ...
      room.statusMessage = `🎉 ${currentActivePlayer.name} WON THE MATCH! 🎉`;
    ...
    if (room.diceValue === 6 || captured !== null) {
      ...
      room.statusMessage = `${currentActivePlayer.name} earned a bonus turn!`;
      setTimeout(() => io.to(roomCode).emit('gameStateUpdate', room), 600);
    } else {
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Ludo Royale Server online on port ${PORT}`);
});
