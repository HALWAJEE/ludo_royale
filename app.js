...
let ws=null, selfId=null, state=null, moving=false, diceTimer=null, audioCtx=null, noiseBuffer=null;
...
function pawnEl(pi,pawn,pos,selectable){
  ...
  if(selectable){const ring=document.createElement('div');ring.className='glow-ring';ring.style.color=`var(--${colors[pi]})`;p.appendChild(ring);p.onclick=()=>{unlockAudio();send({type:'move',pawn});};}
  return p;
}
function renderPawns(){
  ...
  state.game.pawns.forEach((arr,pi)=>arr.forEach((pos,pawn)=>{
    if(pos===0){
      const home=document.querySelector(`.home-box.${colors[pi]}`); const slot=home?.children[pawn];
      const selectable=pi===myIdx&&pi===turn&&state.game.awaitingMove&&validClientMove(pos,dice);
      if(slot){
        if(selectable){const ring=document.createElement('div');ring.className='glow-ring';ring.style.color=`var(--${colors[pi]})`;slot.appendChild(ring);}
        slot.appendChild(pawnEl(pi,pawn,pos,selectable));
      }
    } else if(pos===58){
      const center=document.querySelector('.cell[data-rc="7,7"]'); if(center)center.appendChild(pawnEl(pi,pawn,pos,false));
    } else {
      const rc=pawnCoord(pi,pos); const cell=document.querySelector(`.cell[data-rc="${rc[0]},${rc[1]}"]`);
      if(cell){const selectable=pi===myIdx&&pi===turn&&state.game.awaitingMove&&validClientMove(pos,dice); if(selectable){const ring=document.createElement('div');ring.className='glow-ring';ring.style.color=`var(--${colors[pi]})`;cell.appendChild(ring)} cell.appendChild(pawnEl(pi,pawn,pos,selectable));}
    }
  }));
}
...
function createMotionPawn(pi){
  const board=$('#board'),p=document.createElement('div');p.className=`pawn ${colors[pi]} motion-pawn`;board.appendChild(p);return p;
}
function setMotionPoint(el,point){if(!point)return;el.style.left=`${point.x}px`;el.style.top=`${point.y}px`;}
function animateMove(m){
  const finalState=m.state; const pi=m.player,pawn=m.pawn,from=m.from,to=m.to;
  state=JSON.parse(JSON.stringify(finalState));
  moving=true;$('#rollBtn').disabled=true;
  // Rebuild the board with the moving pawn temporarily removed from its final location.
  state.game.pawns[pi][pawn]=from;
  render();
  const motion=createMotionPawn(pi); const start=boardPoint(pi,from,pawn); const end=boardPoint(pi,to,pawn);
  if(!start||!end){motion.remove();state=finalState;moving=false;render();return;}
  const positions=[];
  if(from===0){positions.push(start);}
  for(let pos=Math.max(1,from+1);pos<=to;pos++)positions.push(boardPoint(pi,pos,pawn));
  if(to===58 && !positions.length)positions.push(end);
  if(!positions.length){motion.remove();state=finalState;moving=false;render();return;}
  motion.style.transform='translate(-50%,-50%)';setMotionPoint(motion,start);
  let index=0;const stepMs=190;
  const moveNext=()=>{
    if(index>=positions.length){
      motion.classList.add('landing');
      setTimeout(()=>{
        motion.remove();state=finalState;moving=false;render();
        if(m.captured?.length){soundCapture();}
        if(to!==58 && to>=1 && to<=52){const landed=(starts[pi]+to-1)%52;if(safe.has(landed)){const cell=document.querySelector(`.cell[data-rc="${path[landed][0]},${path[landed][1]}"]`);cell?.classList.add('star-landed');setTimeout(()=>cell?.classList.remove('star-landed'),700);soundStar();}}
        if(finalState.game.winner!=null)soundWin();
      },130);return;
    }
    const pt=positions[index++];setMotionPoint(motion,pt);soundStep();setTimeout(moveNext,stepMs);
  };
  // Initial step out of the home is intentionally quick and tactile.
  setTimeout(moveNext,60);
}
