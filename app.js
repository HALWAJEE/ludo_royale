...
function addBoardPawn(cell,pi,pawn,pos,selectable,count,index){
  ...
  // Keep the pawn itself at a fixed, board-relative size.
  p.classList.add('board-pawn');
  p.style.width = count === 1 ? '62%' : '39%';
  p.style.height = count === 1 ? '62%' : '47%';
  p.style.left = `${x}%`;
  p.style.top = `${y}%`;
  p.style.transform = 'translate(-50%,-50%)';
  p.style.zIndex = String(30 + index);
  ...
  if(selectable){
    ...
    ring.style.left = `${x}%`;
    ring.style.top = `${y}%`;
    ring.style.width = `${count === 1 ? 72 : 49}%`;
    ring.style.height = `${count === 1 ? 72 : 57}%`;
    ring.style.zIndex = String(25 + index);
...
function renderPawns(){
  ...
  state.game.pawns.forEach((arr,pi)=>arr.forEach((pos,pawn)=>{
    // During an animated move, leave exactly one visual motion pawn in charge.
    if(animatingPawn && animatingPawn.pi===pi && animatingPawn.pawn===pawn) return;

    const selectable =
      pi===myIdx &&
      pi===turn &&
      state.game.awaitingMove &&
      validClientMove(pos,dice);
  ...
  groups.forEach(group=>{
    group.forEach((item,index)=>{
      addBoardPawn(
        item.cell,item.pi,item.pawn,item.pos,
        item.selectable,group.length,index
      );
    });
  });
}
...
function animateMove(m){
  ...
  // Keep the moving pawn out of the normal renderer for the entire animation.
  ...
  motion.style.transform='translate3d(-50%,-50%,0)';
  ...
  if(!points.length) points.push(end);

  let segment=0;
  let fromPoint=start;
  let toPoint=points[0];
  let segmentStart=performance.now();
  const duration=Math.max(120,Math.min(180,820/Math.max(1,points.length)));
    ...
    if(to!==58 && to>=1 && to<=52){
      ...
      if(safe.has(landed)){
        const cell=document.querySelector(
          `.cell[data-rc="${path[landed][0]},${path[landed][1]}"]`
        );
  ...
  function frame(now){
    ...
    // Tiny hop only; never scale the pawn dramatically.
    const lift=Math.sin(Math.PI*t)*Math.min(7,Math.max(3,8-points.length*.35));
    motion.style.left=`${x}px`;
    motion.style.top=`${y}px`;
    motion.style.transform=
      `translate3d(-50%,calc(-50% - ${lift}px),0)`;
    ...
    fromPoint=points[segment-1];
    toPoint=points[segment];
...
function boot(){
  try{
    ...
    console.error('Ludo Royale boot error:',err);
    const msg=document.createElement('div');
    msg.style.cssText='position:fixed;inset:0;display:grid;place-items:center;padding:24px;background:#071127;color:white;font:600 16px system-ui;z-index:99999;text-align:center';
    msg.textContent='Ludo Royale could not start. Please refresh the page.';
