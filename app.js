...
let ws=null,selfId=null,state=null,previousState=null,animating=false;
...
function connect(){
 ...
 if(ws && ws.readyState===WebSocket.CONNECTING && socketReady) return socketReady;
 socketReady=new Promise((resolve,reject)=>{
  ws=new WebSocket(wsUrl());
  ws.onopen=()=>{ setMsg(""); resolve(); };
  ws.onmessage=e=>handle(JSON.parse(e.data));
  ws.onerror=()=>{ setMsg("Connection problem. Please refresh and try again."); reject(new Error("WebSocket connection failed")); };
  ws.onclose=()=>{ if(state) setMsg("Disconnected from game server. Refresh to reconnect."); };
 });
 return socketReady;
}
function send(m){
...
async function enterRoom(type){
 ...
 setMsg("Connecting to the game server…");
 try{
  await connect();
  if(type==="create") send({type:"create",name});
  else{
   const room=$("#roomInput").value.trim().toUpperCase();
   if(!room){setMsg("Enter a room code.");create.disabled=false;join.disabled=false;return;}
   send({type:"join",roomId:room,name});
  }
 }catch(e){
  create.disabled=false;join.disabled=false;
  setMsg("Could not connect to the server. Check the Render deployment and refresh.");
 }
}
