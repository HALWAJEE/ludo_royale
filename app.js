...
const chatForm=$('#chatForm'), chatInput=$('#chatInput'), chatMessages=$('#chatMessages'), chatPanel=$('#chatPanel'), chatToggle=$('#chatToggle');
...
chatForm.onsubmit=(e)=>{e.preventDefault();const text=chatInput.value.trim();if(!text)return;unlockAudio();send({type:'chat',text});chatInput.value='';chatInput.focus();};
chatToggle.onclick=()=>{chatPanel.classList.toggle('collapsed');chatToggle.textContent=chatPanel.classList.contains('collapsed')?'+':'−';};
$('#rollBtn').onclick=()=>{
  if(moving) return;
...
function handle(m){
  ...
  if(m.type==='joined'){
    ...
    actionQueue=[]; clearChat(); showRoom(); return;
  }
  if(m.type==='resumed'){
    selfId=m.selfId; state=m.state; resumeToken=m.resumeToken||resumeToken; savedRoomId=state.roomId;
    localStorage.setItem('ludoResumeToken',resumeToken); localStorage.setItem('ludoRoomId',savedRoomId);
...
function appendChat(msg){
  ...
  el.className='chat-bubble'+(msg.playerId===selfId?' mine':'');
  const name=document.createElement('span'); name.className='chat-name'; name.textContent=msg.playerId===selfId?'YOU':msg.name;
  const body=document.createElement('span'); body.textContent=msg.text;
  el.append(name,body); chatMessages.appendChild(el);
  while(chatMessages.children.length>80) chatMessages.removeChild(chatMessages.firstChild);
  chatMessages.scrollTop=chatMessages.scrollHeight;
}
function clearChat(){if(chatMessages)chatMessages.innerHTML='';}
